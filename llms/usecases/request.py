...
from typing import Any, List, TypedDict, NotRequired, Awaitable
...
class LlmRequest:
    llm: ChatOpenAI
    get_short_conversation_history: Callable[[int, int], list[BaseMessage]]
    get_long_conversation_history: Callable[[int, int], list[dict[str, Any]]]
    ...
    def _build_system_context(self, long_history: List[dict[str, Any]]) -> str:
        context_parts = ["You are a helpful AI assistant. Based on previous conversations:"]
    ...
    async def _get_long_history_node(self, state: ConversationState) -> ConversationState:
        user_id = state["user_id"]
        topic_id = state["topic_id"]
        long_history = await self.get_long_conversation_history(user_id, topic_id)
    ...
    def _create_conversation_graph(self) -> StateGraph:
        ...
        # Define the flow: run history nodes sequentially to avoid concurrent updates
