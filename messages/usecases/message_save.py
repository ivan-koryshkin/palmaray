...
class SaveMessage:
    ...
    async def __call__(self, *, message_id: int, chat_id: int, text: str, topic_id: int, role: RoleEnum) -> int:
        ...
        msg = await self.repo.create(
            MessageModel(id=message_id, chat_id=chat_id, text=encrypted, role=role.value, topic_id=topic_id)
        )
        return msg.id
